...
public class ProductServiceController {
	...
	public ResponseEntity<Object> getProductsByProdname(@RequestHeader("Authorization") String token,
			@PathVariable String productname) throws InvalidTokenException{
		log.info("inside product service controller to get product by prodname");
		if(authfeign.getAdminValidity(token).getBody().getValid()) {
		return productservice.getProductsByProdName(productname);
		}
		throw new InvalidTokenException("Token Expired or Invalid , Login again ...");
	}
